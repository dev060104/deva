...
export default function Birthday3DScene({
  ...
  // Sync activeTheme with prop
  useEffect(() => {
    setActiveTheme(currentTheme);
  }, [currentTheme]);

  // Color palletes for themes
  const themeConfigs = {
    'royal-gold': {
      bg: 0x070913,
  ...
  useEffect(() => {
    ...
    // Ground platform (Glossy reflective pedestal)
    ...
    // Box Lid (Group so it can pivot and lift)
    ...
    // 6. Build Surprise (Tiered Birthday Cake or Chosen Gift) inside Box
    const surpGrp = new THREE.Group();
    surpGrp.position.set(0, 0.2, 0);
    surpGrp.scale.set(0.001, 0.001, 0.001); // Hidden inside until unboxed
    ...
    if (giftType === 'cake') {
      ...
      const cakeMat = new THREE.MeshStandardMaterial({ color: 0xfff5ea, roughness: 0.4 }); // Vanilla cream
      const t1 = new THREE.Mesh(t1Geo, cakeMat);
      t1.castShadow = true;
      t1.position.y = 0.25;
      surpGrp.add(t1);

      // Chocolate / Strawberry Icing Accent
      ...
      const t2Mat = new THREE.MeshStandardMaterial({ color: 0xffccd5, roughness: 0.3 }); // Strawberry pink
      const t2 = new THREE.Mesh(t2Geo, t2Mat);
      t2.castShadow = true;
      t2.position.y = 0.72;
      surpGrp.add(t2);

      // Decorative pearls on tier 2
    ...
    // 8. Stardust / Glitter Particle Field
    const starCount = 450;
    ...
    // 9. Interactive Mouse / Touch Events
    ...
    const animate = () => {
      ...
      // Gentle floating animation of gift box
      if (boxGroup) {
      ...
      // Candle flames flicker (if not blown)
  ...
  // Handle Interactive Unboxing Action
  const triggerUnbox = () => {
    if (isUnboxed) return;
    setIsUnboxed(true);
    ...
    // Animate Lid opening and Surprise emerging
        ...
        <div className="flex items-center gap-2.5">
          {!isUnboxed ? (
            <button
              ...
              className="flex items-center gap-2 px-5 py-3 rounded-2xl bg-gradient-to-r from-amber-500 via-pink-500 to-purple-600 hover:from-amber-400 hover:to-purple-500 text-white text-xs sm:text-sm font-bold shadow-xl shadow-pink-500/25 hover:scale-105 active:scale-95 transition-all"
            >
            ...
            <button
              onClick={blowCandles}
              className="flex items-center gap-2 px-5 py-3 rounded-2xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-white text-xs sm:text-sm font-bold shadow-xl shadow-cyan-500/25 hover:scale-105 active:scale-95 transition-all animate-bounce"
            >
          ...
          <button
            ...
            className="p-3 rounded-2xl bg-black/60 hover:bg-black/80 backdrop-blur-md border border-white/10 text-amber-300 hover:text-amber-200 transition-all shadow-lg flex items-center gap-1.5 text-xs font-semibold"
          ...
          {isUnboxed && (
            <button
              onClick={resetBox}
              className="p-3 rounded-2xl bg-black/60 hover:bg-black/80 backdrop-blur-md border border-white/10 text-slate-300 hover:text-white transition-all shadow-lg"
              title="Reset 3D Box"
            >
        ...
        <div className="flex items-center gap-1.5 bg-black/60 backdrop-blur-md p-1.5 rounded-2xl border border-white/10">
          ...
          {(['royal-gold', 'cosmic-nebula', 'sakura-pastel', 'cyberpunk'] as BirthdayTheme[]).map((thm) => (
            <button
              ...
              className={`px-2.5 py-1.5 rounded-xl text-[11px] font-semibold transition-all capitalize ${
                activeTheme === thm
