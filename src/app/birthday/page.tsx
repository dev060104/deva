...
import { 
  ...
  Flame, Music, RefreshCw, Send, Check, Printer, FileText
} from 'lucide-react';
import Birthday3DScene, { BirthdayTheme, GiftType } from '@/components/birthday/Birthday3DScene';
import TrendingWishesSection from '@/components/birthday/TrendingWishesSection';
import ShareGiftModal from '@/components/birthday/ShareGiftModal';
import GreetingCardModal from '@/components/birthday/GreetingCardModal';
...
function BirthdayAppContent() {
  ...
  const [sender, setSender] = useState('Jordan');
  ...
  useEffect(() => {
    ...
    // If loaded from a shared link, trigger a gentle welcome chime
  ...
  return (
    <div className="space-y-10 max-w-6xl mx-auto pb-16">
      ...
      {hasReceivedSharedLink && (
        <div className="p-4 rounded-3xl bg-gradient-to-r from-pink-900/60 via-purple-900/60 to-indigo-900/60 border border-pink-500/40 shadow-2xl flex flex-col sm:flex-row items-center justify-between gap-3 text-center sm:text-left">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <h3 className="text-base sm:text-lg font-bold text-white">
                {sender} has sent a special 3D Birthday Box for {recipient}!
              </h3>
            </div>
          </div>
          <button
            onClick={() => setIsShareModalOpen(true)}
            className="px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-white text-xs font-semibold transition-all whitespace-nowrap"
          >
            Send One Back 🎁
          </button>
        </div>
      ...
      <section className="text-center space-y-4 pt-2">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-gradient-to-r from-amber-500/10 via-pink-500/10 to-indigo-500/10 border border-pink-500/20 text-pink-300 text-xs font-semibold">
          ...
          <span>Next-Gen 3D Vision Birthday Experience</span>
        </div>
        ...
        <p className="text-sm sm:text-base text-slate-400 max-w-2xl mx-auto leading-relaxed">
          Experience interactive 3D gift unboxing, tiered birthday cakes with blowable candles, 3D fireworks, synthesized music-box melodies, and 40+ curated viral wishes.
        </p>
        ...
        <div className="max-w-2xl mx-auto p-2 sm:p-2.5 rounded-2xl bg-slate-900/80 border border-slate-800 shadow-xl flex flex-wrap items-center justify-center gap-2 text-xs">
          ...
          <button
            onClick={() => setIsShareModalOpen(true)}
            className="flex items-center gap-1.5 px-4 py-1.5 rounded-xl bg-gradient-to-r from-pink-600 to-indigo-600 hover:from-pink-500 hover:to-indigo-500 text-white font-bold transition-all hover:scale-105 shadow-md shadow-pink-600/30"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-amber-300 font-medium transition-all"
